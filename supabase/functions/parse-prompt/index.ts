...
// Whitelist — must stay in sync with semantic_layer.yaml + DB_TAG_LABEL (v2.0).
const ALLOWED_TAGS = [
  // 覆蓋率類
  "socket_most",
  "socket_few",
...
可用標籤(v2.0):
- socket_most           → 多數座位有插座（每桌都有 / 整面牆都是插座）
- socket_few            → 少數座位有插座（角落 / 牆邊才有）
- large_table_most      → 多數是大桌（4 人以上桌）
- large_table_few       → 偶有大桌
- wifi_available        → 有 Wi-Fi
- high_cp_value         → 高 CP 值、便宜大碗、物超所值
...
- time_limit            → 不限時 / 久坐（使用者明確想不被趕時）

特殊處理：
- 「想要有插座」→ socket_most（更精確）；不要同時送 socket_few。
- 「方便停車」→ 視語境送 scooter_parking_easy 或 car_parking_easy；都不確定就兩者皆送（OR）。
- 「寵物友善 / 想看貓 / 想看狗」→ 對應 has_resident_cat / has_resident_dog（不要送 pet_friendly，已廢棄）。
...
6. 模糊時段對應規則（若未指定具體時間）：
   ...
   - 「傍晚」→ 今天 18:00
   - 「晚上」/「夜晚」/「夜裡」→ 今天 20:00
   - 「深夜」/「半夜」→ 明天 00:00（已過 23:00 則以下一天為準）
   - 「凌晨」→ 明天 02:00
   注意：上述都是「開放時間過濾」，不是 tag — 即使使用者只說「晚上有開的」，也必須回傳對應 open_at 時間。
7. 若使用者同時提供模糊時段與更明確的時間（例如「晚上 9 點」、「下午 2:30」），以明確時間為準。
8. 若使用者寫「今天晚上」「明天晚上」等，請結合日期 + 模糊時段對應規則推算。
