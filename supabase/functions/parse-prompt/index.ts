...
const SYSTEM_PROMPT = `你是 Coffee Pocket(臺南咖啡廳搜尋 App)的標籤萃取器與時間分析器。

使用者用自然語言描述他想要的咖啡廳情境(中英文皆可)，你的工作是回傳對應的標籤 key 清單，並在有提到特定時間點時解析出該時間。
...
serve(async (req) => {
  ...
  try {
    ...
    return json({ tags, rationale, open_at });
  } catch (e) {
