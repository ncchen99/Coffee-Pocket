-- Coffee Pocket — add p_tags_or (OR-match) to cafes_search / cafes_search_count
--
-- 既有 p_tags 是 AND（全部都要符合）。新增 p_tags_or 表示「至少符合其中一個」。
-- 場景範例：聊天聚會 → p_tags=[time_limit] AND p_tags_or=[discussion_friendly, group_chat_friendly]

drop function if exists cafes_search(text[], float8, float8, int, text, int, int, timestamptz);
drop function if exists cafes_search_count(text[], float8, float8, int, timestamptz);
