-- Expose `slug` on cafe_detail() so the frontend has it after fetching
-- detail-by-uuid (old bookmarks) and can canonicalize the URL to the slug form.
--
-- Body identical to 0015 except for the extra 'slug' key in jsonb_build_object.
