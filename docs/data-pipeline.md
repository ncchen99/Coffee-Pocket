...
工具會把評論與店家資訊存到 `data/reviews/<google_place_id>.json`。加上 `--update-cafe` 時，也會把可用的店家資訊寫回 `cafes`。
...
- 已被 community 鎖定的標籤（`locked_by_community=true`）不會被自動覆寫。
