...

{
  "signals": [
    {
      "type": "socket_available" | "study_friendly" | "discussion_friendly" | "time_limit" | "pet_friendly" | "reservable" | "outdoor_seating" | "group_chat_friendly" | "noise_level" | "wifi_quality" | "seating_availability",
...
- study_friendly：value 可為 null；polarity 必填（positive/negative）。判斷是否適合一人讀書 / 工作 / 久坐。
- discussion_friendly：value 可為 null；polarity 必填（positive/negative）。判斷是否適合 2~3 人**討論 / 開會 / 共同工作**（小組討論場景，需要能講話但不一定要熱鬧）。
...
- reservable：value = true / false / null；polarity 必填。判斷店家是否可事先訂位 / 預約座位。
- outdoor_seating：value = true / false / null；polarity 必填。判斷店家是否有戶外座位 / 室外區 / 露天座位。
- group_chat_friendly：value 可為 null；polarity 必填（positive/negative）。判斷是否適合**多人（3 人以上）社交聚會 / 純聊天**，重點在「容納多人 + 氛圍熱鬧、可放心暢聊」，與 discussion_friendly 區別在於這是放鬆社交而非工作討論。
- noise_level：value = 1~5 整數。**注意這個欄位代表「安靜程度」：5 = 非常安靜（鴉雀無聲）、1 = 非常吵雜（人聲鼎沸／音樂大聲）**。請依評論描述換算到這個方向，不要顛倒。
- wifi_quality / seating_availability：value = 1~5 整數（5 = 最好 / 最充足）。

語意提示（包含繁中關鍵字與表情符號示意，不是窮舉）：
- 插座：「有插座/每桌都有/可以充電/沒有插座」或符號 🔌
- 讀書辦公（study_friendly）：「適合讀書/工作/安靜/桌子大/位置寬/可久坐」或符號 📖、📚、💻 vs 「很吵/限時/不適合久坐」
- 討論開會（discussion_friendly）：「適合小組討論/開會/帶筆電討論/兩三人工作」 vs 「太安靜不敢講話/太吵聽不到」
- 限時：「限時 90 分鐘」「假日限時」「不限時」或時鐘符號 ⏳、⏰
- 寵物友善：「可以帶狗/寵物友善/歡迎毛小孩/有店狗店貓」或動物符號 🐶、🐱、🐾 vs 「謝絕寵物/不能帶寵物入內」
- 訂位（reservable）：「可訂位/接受訂位/可預約/建議先訂位/打電話訂位/線上訂位」或符號 📞、🗓️ vs 「不接受訂位/僅現場排隊/不可預約/採排隊制」
- 戶外座（outdoor_seating）：「戶外座/戶外座位/露天/室外區/戶外區/室外座位」或符號 🌳、⛱️ vs 「沒有戶外座/僅限室內/無室外座位」
- 多人聊天（group_chat_friendly）：「適合多人/朋友聚會/聚餐/包場/有大桌/一群人聊天/熱鬧暢聊/家庭聚會」 vs 「位置太小/不適合多人/太安靜不敢講話/僅限兩人桌/限一人一杯」

注意：同一句話如果同時暗示「適合工作討論」與「適合朋友聚會」，可以同時產生 discussion_friendly 與 group_chat_friendly 兩個 signal；但若只是泛指「適合聊天」，請優先歸到 group_chat_friendly。
